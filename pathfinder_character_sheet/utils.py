...
def make_roll_result_text(rolls: list[int], modifiers: list[int]) -> str:  #23 (line in Coconut source)
    ...
    for i in range(len(rolls)):  #25 (line in Coconut source)
        text.append("Result: {_coconut_format_0} + {_coconut_format_1} = {_coconut_format_2}".format(_coconut_format_0=(rolls[i]), _coconut_format_1=(modifiers[i]), _coconut_format_2=(rolls[i] + modifiers[i])))  #26 (line in Coconut source)
    return _coconut_tail_call("<br>".join, text)  #27 (line in Coconut source)


def get_attack_info(attack_info_str: str) -> _coconut.typing.Tuple[str, str, int]:  #29 (line in Coconut source)
    num_dice, die, modifier = "", "", ""  #30 (line in Coconut source)
    curr_val = ""  #31 (line in Coconut source)
    for char in attack_info_str:  #32 (line in Coconut source)
        if num_dice == "":  #33 (line in Coconut source)
            if char == 'd':  #34 (line in Coconut source)
                num_dice = curr_val  #35 (line in Coconut source)
                curr_val = ""  #36 (line in Coconut source)
            else:  #37 (line in Coconut source)
                curr_val += char  #38 (line in Coconut source)
        elif die == "":  #39 (line in Coconut source)
            if char == '+' or char == '-':  #40 (line in Coconut source)
                die = curr_val  #41 (line in Coconut source)
                curr_val = char  #42 (line in Coconut source)
            else:  #43 (line in Coconut source)
                curr_val += char  #44 (line in Coconut source)
        else:  #45 (line in Coconut source)
            if not char == " ":  #46 (line in Coconut source)
                curr_val += char  #47 (line in Coconut source)
    if not die:  #48 (line in Coconut source)
        die = curr_val  #49 (line in Coconut source)
        modifier = 0  #50 (line in Coconut source)
    else:  #51 (line in Coconut source)
        modifier = curr_val  #52 (line in Coconut source)
    return num_dice, die, modifier  #53 (line in Coconut source)
